...
public class TableWebsocket {
	...
	@OnMessage
	public void onReciveMsg(Session clientSession, String msg, String gid,
			@PathParam("uid") String uid) throws InterruptedException {
		System.out.println("on recive msg." + msg);

		for (Session session : sessions) {
			session.getAsyncRemote().sendText(
					String.valueOf(java.time.LocalTime.now().getSecond()));
		}
...
// }
/*
 * ======= static Set<Session> users = Collections.synchronizedSet(new
 * HashSet<Session>()) ;
 * 
 * 
 * @OnOpen public void handleOpen(Session user){
 * 
 * users.add(user) ; System.out.println("player has joined"); }
 * 
 * @OnMessage public void handleMessage(String message , Session userSession){
 * String username = (String)userSession.getUserProperties().get("username") ;
 * if(username==null){ userSession.getUserProperties().put("username",message) ;
 * // userSession.getBasicRemote().sendText(buildJsonData("System", message)) ;
 * }else{ Iterator<Session> iterator = users.iterator() ; //
 ...
 * }
 * 
 * @OnClose public void handleClose(Session user){ users.remove(user);
 ...
 * ObjectMapper mapper = new ObjectMapper() ;
 * 
 * String[] temp = {"message",username +":" +message}; // String json =
